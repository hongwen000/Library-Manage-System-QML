...
void securityDB::addUserToSecurityDb(const QString &_id,
  ...
  byteArray.append(_password);
  QByteArray hash = QCryptographicHash::hash(byteArray, QCryptographicHash::Md5);
  QString MD5_password = hash.toHex();
  query.addBindValue(MD5_password);
...
void securityDB::editUserToSecurityDb(const QString &_id,
                                      const QString &_password,
                                      const QString &_type) {
  QSqlQuery query(*pDB);
  if (_password != "") {
