...
QVariantHash libraryDB::get() {
  ...
  table.clear();
  query.exec(
      "select isbn, bookName, author, publishDate, totalStock from book");
  while (query.next()) {
    QVariantHash term;
...
void libraryDB::borrowBookDCL(const QString &book, const QString &user,
  ...
  query.clear();
  query.prepare(
      "update book set avaiStock = ?, borrowedNum = ? where isbn = ?");
  query.addBindValue(avaiStock);
  query.addBindValue(borrowedNum);
  query.addBindValue(book);
  if (!query.exec()) {
    qDebug() << "Error Writing DataBase when changing book borrowed and then "
                "borrowed number";
  }
  qDebug() << pDB->lastError().databaseText();
}

void libraryDB::returnBookDCL(const QString &book, const QString &user,
...
void libraryDB::addUserToDb(const QString &_id, const QString &_name,
...
void libraryDB::editUserToDb(const QString &_id, const QString &_name,
                             const QString &_email, const QString &_type) {
  QSqlQuery query(*pDB);
...
void libraryDB::addBookDCL(const QString &_bookName, const QString &_author,
                           const QString &_isbn, int _totalStock,
                           const QString &_publishDate) {
  QSqlQuery query(*pDB);
  query.prepare("insert into book(isbn, bookName, author, publishDate, "
                "totalStock, avaiStock, borrowedNum) values (?, ?, ?, ?, ?, ?, "
                "0)");
...
void libraryDB::editBookDCL(const QString &_bookName, const QString &_author,
                            const QDate &_publishDate, int _totalStock,
                            int _avaiStock) {
  QSqlQuery query(*pDB);
  query.prepare("update book set bookName = ?, author = ?, publishDate = ?, "
                "totalStock = ?, avaiStock = ?, borrowedNum = ?  where id = ?");
  query.addBindValue(_bookName);
  query.addBindValue(_author);
  query.addBindValue(_publishDate);
  query.addBindValue(_totalStock);
