...
class Book : public QObject {
  ...
  Q_PROPERTY(
      QString bookName READ bookName WRITE setBookName NOTIFY bookNameChanged)
  Q_PROPERTY(int type READ type WRITE setType NOTIFY typeChanged)
  Q_PROPERTY(QString author READ author WRITE setAuthor NOTIFY authorChanged)
  Q_PROPERTY(QDate publishDate READ publishDate WRITE setPublishDate NOTIFY
                 publishDateChanged)
  Q_PROPERTY(int totalStock READ totalStock WRITE setTotalStock NOTIFY
                 totalStockChanged)
  Q_PROPERTY(
      int avaiStock READ avaiStock WRITE setAvaiStock NOTIFY avaiStockChanged)
  Q_PROPERTY(int borrowedNum READ borrowedNum WRITE setBorrowedNum NOTIFY
                 borrowedNumChanged)
  Q_PROPERTY(QVariantList borrower READ borrower WRITE setBorrower NOTIFY
                 borrowerChanged)
  Q_PROPERTY(QVariantList borrowDate READ borrowDate WRITE setBorrowDate NOTIFY
                 borrowDateChanged)
public:
  Book(const QString &_isbn = "", int _type = 0, const QString &_bookName = "",
       const QVariantList &_borrower = QVariantList(),
       const QVariantList &_borrowDate = QVariantList(), QString _author = "",
       const QDate &_publishDate = QDate::currentDate(), int _totalStock = 0,
       int _avaiStock = 0, int _borrowedNum = 0)
      : m_isbn(_isbn), m_type(_type), m_bookName(_bookName),
        m_borrower(_borrower), m_borrowDate(_borrowDate), m_author(_author),
        m_publishDate(_publishDate), m_totalStock(_totalStock),
        m_avaiStock(_avaiStock), m_borrowedNum(_borrowedNum) {}
  Book(const Book &rhs) {
    m_isbn = rhs.m_isbn;
  ...
  ~Book() {
    ...
    for (auto usr : m_borrower) {
      qDebug() << "I will erase myself from user "
               << qvariant_cast<User *>(usr)->id();
  ...
  Q_INVOKABLE void edit(const QString &_bookName, const QString &_author,
