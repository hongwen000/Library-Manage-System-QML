...
class User : public QObject {
  ...
  Q_PROPERTY(
      QVariantList record READ record WRITE setRecord NOTIFY recordChanged)

public:
  User(const QString &_id = "", const QString &_password = "",
       const QVariantList &_record = QVariantList(), const QString &_name = "",
       const QString &_email = "", const QString &_type = "user")
      : m_id(_id), m_password(_password), m_record(_record), m_name(_name),
        m_email(_email), m_type(_type) {
  ...
  ~User() { qDebug() << "I am desturcted, help"; }
