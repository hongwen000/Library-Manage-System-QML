...
void Book::bookOutTo(User *_borrower) {
  ...
  borrowBookDCL(this->isbn(), _borrower->id(), tmp.toString("yyyy-MM-dd"),
                m_avaiStock, m_borrowedNum);
}

void Book::bookOutTo(User *_borrower, QDate _borrowDate) {
  // Todo : limit it that only borrow one book
  m_borrower << QVariant::fromValue(_borrower);
  qDebug() << "It's me, book" << bookName() << "been borrowed at"
           << _borrowDate.toString("yyyy-MM-dd");
  // m_borrower.append(_borrower);
  m_avaiStock--;
  m_borrowedNum++;
...
void Book::edit(const QString &_bookName, const QString &_author,
